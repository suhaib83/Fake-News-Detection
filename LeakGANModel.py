...
class LeakGAN(object):
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        with tf.variable_scope('place_holder'):
            ...
            #self.x = tf.placeholder(tf.cast(shape=[self.batch_size,self.sequence_length], dtype=tf.int32))# sequence of tokens generated by generator
            ...
            self.drop_out = tf.placeholder(tf.float32, name="drop_out_rate")
    ...
    def pretrain_step(self, sess, x,drop_out_rate):
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss,self.pretrain_manager_updates,self.pretrain_goal_loss],
                           feed_dict={self.x: x,self.drop_out:drop_out_rate})
        return outputs

    def generate(self, sess,drop_out_rate,train = 1):
        
        outputs = sess.run(self.gen_x,feed_dict={self.drop_out:drop_out_rate,self.train:train})
        return outputs
