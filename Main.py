...
HIDDEN_DIM = 32  
START_TOKEN = 0
PRE_EPOCH_NUM = 80 #80  # supervise (maximum likelihood estimation) epochs
SEED = 88
BATCH_SIZE = 48
GOAL_SIZE = 16
STEP_SIZE = 4
...
TOTAL_BATCH = 5 #100 ***********
positive_file = 'data/text_to_code.txt'
...
def get_reward(model,dis, sess, input_x, rollout_num, dis_dropout_keep_prob,total_epoch,data_loader):
    ...
    for i in range(rollout_num):
        ...
        for given_num in range(1, model.sequence_length // model.step_size):
            ...
            feed = {dis.D_input_x: samples, dis.drop_out_rate: dis_dropout_keep_prob}
        ...
        feed = {dis.D_input_x: samples,  dis.drop_out_rate: 1.0}
        ypred_for_auc = sess.run(dis.ypred_for_auc, feed)
        ypred = np.array([item[1] for item in ypred_for_auc])
        if i == 0:
            rewards.append(ypred)
...
def plot_losses(g_losses, w_losses, d_losses):
    plt.figure()
    plt.plot(g_losses, label='Generator loss')
    plt.plot(w_losses, label='Worker loss')
...
def main():
   ...
   # _, _, _, SEQ_LENGTH, vocab_size = cPickle.load(open(pickle_loc))
                ...
                for _ in range(PRE_EPOCH_NUM):
                    ...
                    for _ in range(3):
                        ...
                        feed = {
                            ...
                            discriminator.drop_out_rate: dis_dropout_keep_prob
                        }
    ...
    print (datetime.now())
    # Call the function
    plot_losses(g_losses, w_losses, d_losses)
